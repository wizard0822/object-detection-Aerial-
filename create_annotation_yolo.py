...
training_set.to_csv('./train_yolo.txt', index=False, header=False)
...
validation_set.to_csv('./vald_yolo.txt', index=False, header=False)
...
testing_set.to_csv('./test_yolo.txt', index=False, header=False)
...
"""
dataset = glob.glob('dataset/SIMS/images/*jpg')
count = 0
for x in dataset:
    #print(x)
    img = cv2.imread(x)
    ...
    if img.shape!=(768,1024,3):
       ...
       count = count + 1
       cv2.imwrite("./outputs/samples/"+str(filename),img)
print("Total Images of Non-Standard Size"+ str(count))
"""